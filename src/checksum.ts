...
function toWordArray(buf: Buffer) {
  return crypto.lib.WordArray.create(buf);
}

function toBuffer(wa: any) {
  return Buffer.from(wa.toString(crypto.enc.Hex), 'hex');
}

function sha256x2(buffer: Buffer) {
  var wa = toWordArray(buffer);
  var tmp = SHA256(wa);
  return toBuffer(SHA256(tmp as any));
}
...
function c32checksum(dataHex: string): string {
  const dataHash = sha256x2(Buffer.from(dataHex, 'hex'));
